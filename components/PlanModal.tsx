...
import { CATEGORY_LABELS, TIERS, STATUS_LABELS, STATUS_STYLES } from '@/lib/plans';
...
type TabKey = 'poster' | 'original' | 'enriched' | 'costs' | 'tips' | 'helpers' | 'chat' | 'tip-form' | 'helper-form';
...
export default function PlanModal({ plan, onClose, onBudgetChange, showCosts = true }: PlanModalProps) {
  ...
  useEffect(() => {
    if (plan) {
      setActiveTab(plan.poster_url ? 'poster' : 'original'); setEnrichedCanvas(null); setCostVersions([]); setCostItems([]);
  ...
  const leftTabs: { key: TabKey; label: string; badge?: number }[] = [
    ...(plan.poster_url ? [{ key: 'poster' as TabKey, label: 'Poster' }] : []),
      ...
      <div style={{ background: 'var(--wh)', borderRadius: 16, maxWidth: 680, width: '100%', height: '85vh', display: 'flex', flexDirection: 'column', animation: 'modalIn 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}>
        ...
        <div style={{ flexShrink: 0 }}>
          <div style={{ padding: '24px 28px 0', position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 24, fontWeight: 400, marginBottom: 4, paddingRight: 40 }}>{plan.name}</h2>
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--sea)', marginBottom: 10 }}>{displayBudget}</div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', fontSize: 13, color: 'var(--ink-m)', marginBottom: 16 }}>
              ...
              {plan.status && STATUS_STYLES[plan.status] && <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 10px', borderRadius: 12, background: STATUS_STYLES[plan.status].bg, color: STATUS_STYLES[plan.status].fg }}>{STATUS_LABELS[plan.status]}</span>}
        ...
        {/* Scrollable body / Poster area */}
        {activeTab === 'poster' && plan.poster_url ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12, minHeight: 0, overflow: 'hidden', position: 'relative' }}>
            <Image src={plan.poster_url} alt={plan.name} fill sizes="(max-width: 680px) 90vw, 600px" style={{ objectFit: 'contain' }} />
          </div>
        ) : (
