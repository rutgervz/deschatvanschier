...
import { CATEGORY_COLORS, CATEGORY_LABELS, TIERS, STATUS_LABELS, STATUS_STYLES } from '@/lib/plans';
...
function generateCloudWords(plans: Plan[]): { t: string; w: number; id: number }[] {
  return plans.filter(p => p.status !== 'niet_actief').map(p => {
...
export default function MogelijkmaakdagPage() {
  ...
  const activePlans = plans.filter(p => p.status !== 'niet_actief');
    ...
    <>
      ...
      <section style={{
        ...
        <p style={{
          ...
          Hier kun je de plannen vinden die op de Mogelijkmaakdag op 1 april 2026 in De Stag een sleutel in hun schatkist hebben ontvangen van de Sleuteltafel. Uit deze projecten worden binnenkort door de kinderen hun favorieten gekozen, zodat de €&nbsp;60.000 euro subsidie van de Buitenkans Junior kan worden toegekend.
        </p>
      ...
      <section id="plannen" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 2rem' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10, maxWidth: 1000 }}>
              {filteredPlans.map(p => {
                const cs = CAT_STYLES[p.category] || { bg: 'var(--sand)', fg: 'var(--ink-l)' };
                const displayBudget = p.active_budget || p.budget;
                return (
                  <div key={p.id} onClick={() => setSelectedPlan(p)} style={{
                    display: 'flex', background: 'var(--wh)', border: '1px solid var(--border)', borderRadius: 8,
                  ...
                  >
                    <div style={{ width: 110, flexShrink: 0, aspectRatio: '3/4', overflow: 'hidden', background: 'var(--sand)', alignSelf: 'stretch', position: 'relative' }}>
                      {p.poster_url && <Image src={p.poster_url} alt={p.name} fill sizes="110px" style={{ objectFit: 'cover' }} />}
                    </div>
                    <div style={{ padding: '8px 10px', flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.3, marginBottom: 2 }}>{p.name}</div>
                      <div style={{ fontSize: 13, color: 'var(--sea)', fontWeight: 600, marginBottom: 3 }}>{displayBudget}</div>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 3 }}>
                        <span style={{ display: 'inline-block', fontSize: 7, fontWeight: 600, padding: '2px 5px', borderRadius: 5, background: cs.bg, color: cs.fg, textTransform: 'uppercase', letterSpacing: 0.3 }}>{CATEGORY_LABELS[p.category]}</span>
                        {p.status && STATUS_STYLES[p.status] && (
                          <span style={{ display: 'inline-block', fontSize: 7, fontWeight: 600, padding: '2px 5px', borderRadius: 5, background: STATUS_STYLES[p.status].bg, color: STATUS_STYLES[p.status].fg }}>{STATUS_LABELS[p.status]}</span>
                        )}
                      </div>
                      <div style={{ fontSize: 10, color: 'var(--ink-m)', lineHeight: 1.3 }}>{p.team.join(', ')}</div>
                    </div>
                  </div>
                );
              })}
      ...
      <section id="budget" style={{ maxWidth: 1100, margin: '0 auto', padding: '4rem 2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {TIERS.map(tier => {
            const items = activePlans.filter(p => (p.active_tier || p.tier) === tier.n);
      ...
      {/* SLEUTELS KNOP */}
      <section style={{ textAlign: 'center', padding: '2rem', marginTop: '1rem' }}>
        <button onClick={() => {
          const pw = prompt('Voer het wachtwoord in:');
          if (pw === 'SchatvanSchier@42!') {
            window.location.href = '/mogelijkmaakdag/sleutels';
          } else if (pw !== null) {
            alert('Onjuist wachtwoord');
          }
        }} style={{
          padding: '14px 40px', borderRadius: 50, border: 'none', cursor: 'pointer',
          background: 'var(--dune)', color: '#fff', fontFamily: "'Outfit', sans-serif",
          fontSize: 16, fontWeight: 600, transition: 'transform 0.15s',
        }}
          onMouseOver={e => { e.currentTarget.style.transform = 'scale(1.03)'; }}
          onMouseOut={e => { e.currentTarget.style.transform = ''; }}
        >🗝️ Mijn sleutels</button>
      </section>
